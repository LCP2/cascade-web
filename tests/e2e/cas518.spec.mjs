// CAS-518: the old app icon mark (rising bars + agent arc, gradient id "cg") is replaced by the
// "Cascading Frames" design — three fanned film frames with sprocket dots, a play triangle in the front
// frame, on a deep violet background — in the favicon <link>s (CAS-538 removed the header's own inline
// copy of this mark). This checks the new gradient ids are present and the old ones are gone, and that
// the icon links still resolve.
