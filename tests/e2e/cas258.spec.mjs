...
test("CAS-258: all five windows are named, Premium (CAS-359) included", async ({ page }) => {
  await freshApp(page);
  const labels = await page.locator(".splashloz .splashpill").allTextContents();
  expect(labels.map(s => s.trim())).toEqual(["Upcoming", "In cinemas", "Premium", "Rental", "Streaming"]);
});
