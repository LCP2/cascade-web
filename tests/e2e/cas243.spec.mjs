// CAS-243: the streaming agent's pay-per-film windows are two, and Premium is not one you are opted into.
import { test, expect } from "@playwright/test";
import { toShortlist, shortlistCards, pickCard, finishFlow, toListing } from "./helpers.mjs";

test("CAS-243: Streaming settings offer Premium, Standard Rent and Streaming, each with both switches", async ({ page }) => {
  ...
  expect(lanes.map(l => l.name)).toEqual(["Premium", "Standard Rent", "Streaming"]);
  for(const l of lanes) expect(l.switches.length, `${l.name} must carry List and Notify`).toBe(2);

  // The onboarding assumption: Premium off, the other two on.
  expect(lanes.find(l => l.name === "Premium").on, "a new agent must not be opted into ~$30 premium").toBe(false);
  ...
  // Switching Premium on really widens the agent, and arms the bell that goes with it.
  ...
  await page.locator("#wwLanes .wwlane", { hasText: "Premium" }).locator(".agwt", { hasText: "Notify" }).click();
