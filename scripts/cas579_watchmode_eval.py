...
CAS-766: Q6's field-coverage sample can optionally be drawn from a release-date
window (CAS579_SAMPLE_START / CAS579_SAMPLE_END, YYYYMMDD) instead of always
being the newest releases — the original 1/8 critic_score reading was drawn
from unreleased 2026 titles, which cannot have a critic score yet.
"""
...
SAMPLE_START = os.environ.get("CAS579_SAMPLE_START")  # YYYYMMDD, optional
SAMPLE_END = os.environ.get("CAS579_SAMPLE_END")  # YYYYMMDD, optional
SAMPLE_SIZE = 40  # CAS-766: was 8 - too few to separate thin coverage from an unlucky draw

spent = 0
report = []


def sample_query_params(start=None, end=None, limit=250):
    """Params for the /list-titles/ call Q6's field-coverage sample is drawn from.

    With no window, this is byte-for-byte the same request Q3's `disc` call already
    makes (AU movies, newest-first, limit 250) - an unparameterised run must not change
    what titles get sampled, only how many of them (SAMPLE_SIZE) get evaluated.
    """
    params = {"regions": "AU", "types": "movie", "sort_by": "release_date_desc", "limit": limit}
    if start:
        params["release_date_start"] = start
    if end:
        params["release_date_end"] = end
    return params


def describe_sample_window(start, end):
    if not start and not end:
        return "no window set (most recent AU releases)"
    return f"{start or 'earliest'}..{end or 'latest'}"
...
def main():
    ...
    say()
    window_desc = describe_sample_window(SAMPLE_START, SAMPLE_END)
    if SAMPLE_START or SAMPLE_END:
        sample_disc, sample_err = get("/list-titles/", sample_query_params(SAMPLE_START, SAMPLE_END))
    else:
        sample_disc, sample_err = disc, err
    sample_titles = ((sample_disc or {}).get("titles") or [])[:SAMPLE_SIZE]
    sample = [t.get("id") for t in sample_titles]
    years = sorted({t.get("year") for t in sample_titles if t.get("year")})
    year_range = f"{years[0]}–{years[-1]}" if years else "unknown"
    if sample_err and not sample:
        say(f"FAILED: {sample_err}")
    if not sample:
        say("No sample ids (discovery failed or the window returned nothing).")
    else:
        say(f"Sample window requested: **{window_desc}** · release years actually sampled: "
            f"**{year_range}** · titles: **{len(sample)}**")
        say()
