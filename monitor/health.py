...
evidence the night's work actually happened. This module asserts ten concrete things about
...
def check_watchmode_fetch(stats: dict | None) -> dict:
    base = _check_fetch("watchmode_fetch", stats)
    if base["ok"] is not True:
        return base
    remaining = (stats or {}).get("remaining_monthly_credits")
    if remaining is None:
        return _check("watchmode_fetch", None, base["value"], WATCHMODE_MIN_REMAINING,
                      "calls/errors OK but remaining_monthly_credits unavailable.")
    if remaining < WATCHMODE_MIN_REMAINING:
        return _check("watchmode_fetch", False, remaining, WATCHMODE_MIN_REMAINING,
                      f"only {remaining} Watchmode credit(s) left this month "
                      f"(floor {WATCHMODE_MIN_REMAINING}).")
    return _check("watchmode_fetch", True, base["value"], WATCHMODE_MIN_REMAINING,
...
CHECK_NAMES = ("catalogue_size", "catalogue_integrity", "tmdb_fetch", "watchmode_fetch",
              "oscarbase_fetch", "score_coverage", "email_send", "push_send",
              "usage_events_insert", "auth_signin")


def run_checks(*, today_movies, prev_movies, stats, usage_probe, auth_probe, apns_configured) -> list:
    return [
        check_catalogue_size(today_movies, prev_movies),
        check_catalogue_integrity(today_movies),
        check_tmdb_fetch(stats.get("tmdb")),
        check_watchmode_fetch(stats.get("watchmode")),
...
def _dry_run_inputs():
    ...
    return today_movies, prev_movies, stats, usage_probe, auth_probe, True
...
def main(argv=None) -> int:
    ...
    if args.dry_run:
        today_movies, prev_movies, stats, usage_probe, auth_probe, apns_configured = _dry_run_inputs()
    else:
